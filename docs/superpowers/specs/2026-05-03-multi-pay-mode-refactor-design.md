...
> 日期：2026-05-03  
> 状态：待审批  
> 基于：PLAN.md v2.0 + spec-multi-pay-mode.md + 全项目代码审计
...
/**
 * 根据 overtime_type 获取对应费率
 */
export function getRateByType(overtimeType, project, salaryConfig) {
  const key = overtimeType + '_rate' // weekday_rate | weekend_rate | holiday_rate
...
export function calcPay(record, project, salaryConfig) {
  switch (record.pay_mode) {
    ...
    case 'hourly':
    default: {
      const rate = record.rate || getRateByType(record.overtime_type || 'weekday', project, salaryConfig)
...
- `stores/overtimeStore.js` — 删除 `calculatePay()` 方法
...
### 4.2 删除 `utils/holidays.js`

逻辑全部迁入 `holidayStore`，原文件删除。所有引用 `getOvertimeType` 的页面改为：
```js
import { useHolidayStore } from '@/stores/holidayStore'
const holidayStore = useHolidayStore()
holidayStore.getOvertimeType(dateStr)
```

### 4.3 修改 `stores/overtimeStore.js`
...
| 修复 `addRecord()` 重复 `pay_mode` 赋值 | 删除第二次 `pay_mode: payMode` |
...
### 5.1 record.vue（7 项修复）

| # | 问题 | 修复方案 | 关键代码位置 |
|---|------|---------|------------|
| R1 | 编辑模式不加载项目列表 | `onLoad` 中无论编辑还是新建都调 `loadProjectPicker()`；编辑时回填 `selectedProjectId` | `onLoad(options)` 方法 |
| R2 | 补贴/扣款 UI 缺失 | 在三种模式 template 各加一个"补贴 & 扣款"折叠区：夜班补贴/餐补/交通补贴 三个 input + 扣款金额/原因 | 各模式 template 底部 |
| R3 | 结算 toggle 缺失 | 表单底部加 Switch 行："已结算"，v-model 绑定 `settled` | 备注下方 |
| R4 | 常用短语 UI 缺失 | 备注输入框下方加横向滚动的短语标签行，点击填入备注 | 备注下方 |
| R5 | addRecord `pay_mode` 重复 | 删除 `addRecord` 中的第二次 `pay_mode: payMode` 赋值 | overtimeStore.js:137 |
| R6 | "今日已记"标签误导 | 改为"{{ displayDate }} 已记"，明确按选中日期统计 | `monthDailyCount` label |
| R7 | 日薪/计件零费率校验 | `handleSave` 中日薪模式加 `daily_rate <= 0` 检查，计件加 `piece_rate <= 0` 检查，弹 Toast | `handleSave()` 方法 |
...
### 5.5 salary.vue（1 项修复）

| # | 问题 | 修复方案 |
|---|------|---------|
| A1 | 重算依赖云函数 | 改为本地重算：遍历当月 records，用 `calculator.calcPay` + `calculator.calcNetPay` 重算，直接更新 localStore |

### 5.6 project-edit.vue（2 项修复）
...
### 5.7 projects.vue（1 项修复）

| # | 问题 | 修复方案 |
|---|------|---------|
| P3 | 删除功能无 UI | 每张项目卡片右侧加删除小图标（🗑），点击弹出确认弹窗，确认后调 `confirmDelete` |

### 5.8 batch-record.vue（1 项修复）
...
`getMonthlySummary` 和 `getYearStats` 需增加 `totalDays`、`totalQuantity` 字段。此项为服务端优化，不影响客户端主流程，可放在最后处理。
...
Phase 3: 页面修复（按 P0→P1→P2）
  ...
  └─ P3: 云函数 summary/stats 补充字段（1 项）

Phase 4: 全量回归验证
  └─ 3 种计薪模式各走通：新建项目 → 记工 → 首页查看 → 统计 → 对账
```
